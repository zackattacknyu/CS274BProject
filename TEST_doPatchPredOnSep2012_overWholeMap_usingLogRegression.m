...
for n = 1:N
   ...
   curXdata = feats_test{n}(:,1);
   curTempMap = reshape(curXdata,sizr,sizc);
   curYdata = floor(labels_test{n});
   ...
   for nn = 1:length(patchCoords{n})
      ...
      tempPatch = curTempMap(minR:maxR,minC:maxC);
      labelPatch = curYdata(minR:maxR,minC:maxC);
      testPixels = find(labelPatch>1);
      
      XX2 = tempPatch(testPixels);
      YY2 = categorical(labelPatch(testPixels)-2);
      YHAT = mnrval(bb,XX2);
      YHAT2 = YHAT(:,2);
      curPatch = zeros(curSizeR,curSizeC);
      curPatch(testPixels) = YHAT2;

      curProb3wholeMap(minR:maxR,minC:maxC) = curPatch;
   end
...
%%
save('ROCvars_sep2012_logRegPatched_testInds_wholeMap_unfilteredP.mat',...
    'rocx','rocy','rocThr','rocAuc',...
    'probDet','falseAlarm','thr','auc','trialInds2','missRate','specRate','thr2','auc2');

save('ROCvarsNeg_sep2012_logRegPatched_testInds_wholeMap_unfilteredP.mat',...
